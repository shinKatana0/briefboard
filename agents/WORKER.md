...
- Implement exactly what is in the brief. If you discover a new problem along the way —
  ...
  Before that `add` (only for a finding you filed yourself — not for
  a task the orchestrator already assigned: it already exists, it needs no new ID) —
  if in your current environment you have access to check against the
  current state of `main` (for example `git fetch origin main` and
  viewing `doc/backlog.md` from it, or any other means available in your
  environment) — do so, to pick the freshest possible
  "next number" rather than relying only on the snapshot of your isolated
  branch. This reduces, but does not eliminate, the ID race between parallel
  workers: the final duplicate check at merge time is still done by the
  orchestrator.
