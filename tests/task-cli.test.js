...
describe('task.mjs start and review-start (T-0319, T-0320)', () => {
  ...
  // The two outcomes where the board DID write the transition and the agent is
  // still missing. They are not refusals, and the command must not let them read
  // as ones.
  describe('the transition happened and the session did not', () => {
    it('a project that is not a git repository: the status moved, the exit code says no session', async () => {
      const root = makeTmpRoot();
      const id = readyTask(root);
      const before = bytes(root);
      ...
      assert.strictEqual(doc.status, 'in_progress', 'the board wrote the transition');
      ...
      // once where the file genuinely DOES change: a comparison that could not
      // fail would be confirming the fixture and saying nothing about the code
      // (T-0182). It is what makes review-start's "byte-identical" mean something.
