...
директорию и создаст пустые `doc/backlog.md` + `doc/brief/`. В проект, который
уже существует, она дозаполняет недостающее пофайлово, сохраняет каждый уже
имеющийся файл, а существующие `CLAUDE.md` / `AGENTS.md` дополняет своими
инструкциями — отдельным размеченным блоком, вместо того чтобы их заменять:
...
- Карточка в «Ревью» несёт на себе конец работы: доска спрашивает у git, влита
  ...
  остаётся вашим.
- Та же карточка несёт и второй конец: «Вернуть на доработку» делает переход
  `review` → `in_progress` и запускает воркер-сессию на той самой ветке, на
  которой уже лежит предыдущий круг. Здесь тоже спрашивается подтверждение —
  запускается агент, — а без ветки `task/T-NNNN` действие отказывает: доработка
  начала бы с HEAD и потеряла бы тот круг, который должна была исправить.
  Отсутствующий *worktree* отказом не является: он воссоздаётся из ветки. Сам
  переход был разрешён всегда; новое здесь — то, что на задачу кого-то ставят,
  чего `status … in_progress` не делал никогда.
- Карточка в «В работе» несёт **«Продолжить работу»**: снова воркер-сессия, на той
  ветке, на которой задача уже стоит, — для сессии, которая закончилась, а задача
  никуда не сдвинулась: прерванная доска, упавший воркер, перезагруженная машина.
  Статуса она не пишет, потому что карточка и так там, где надо, и отказывает,
  пока по задаче действительно идёт сессия (это читается из реестра сессий, а не
  угадывается по статусу) или когда нет ветки `task/T-NNNN`, которую можно было бы
  продолжить. Пока этого не было, единственный путь обратно к воркеру отсюда лежал
  через вопрос, которого сессия не задавала, вписанный в описание.
...
«В работе». Кнопки на карточках запускают их тоже: «Запустить ревью-сессию» на
карточке в «Ревью» запускает третий род, **ревью**-сессию; «Вернуть на доработку»
на той же карточке — снова воркер-сессию, на второй круг по той же ветке; а
«Продолжить работу» на карточке в «В работе» запускает её для умершей сессии, не
двигая карточку.
...
- `BRIEFBOARD_SETUP_CMD` — команда, которая делает свежий worktree пригодным для
  ...
  (`BRIEFBOARD_SETUP_TIMEOUT_MS` ниже) убивают команду и отменяют сессию — агент,
  выпущенный в неподготовленный checkout, отчитывается о сбоях, которые не
  относятся к его задаче. Причина и собственный вывод команды уходят в лог сессии
  строкой
...
- `BRIEFBOARD_SETUP_TIMEOUT_MS` — сколько этой команде отведено, в миллисекундах
  (по умолчанию `600000`, десять минут). Умолчание здесь — потолок, а не
  измерение: briefboard никогда не запускал вашу команду установки и оценить её
  не может, так что если вы знаете, что ваша столько честно занимать не может, —
  скажите это здесь, и зависшая установка откажет в сессии за секунды вместо
  того, чтобы держать дроп десять минут. Пустое значение и всё, что не является
  положительным числом, откатывается к умолчанию, а не снимает ограничение, — то
  же правило, что у `BRIEFBOARD_SESSION_MAX`.
...
- неудачный запуск сессии не откатывает переход — карточка остаётся там, куда вы
  её перетащили, а сбой сообщается в ответе; сервер от этого не падает. Дроп в
  «В работе» — единственное исключение, и дело тут в том, что означает статус:
  отправка, так и не ставшая сессией, возвращает задачу в `ready`, потому что
  `in_progress` говорит, что на задаче есть агент, а не что кто-то попытался его
  поставить. Задача, по которой сессия уже идёт, остаётся на месте, и остаётся
  задача на доске без воркер-команды — там дроп означает, что человек берёт
  задачу руками;
...
- **`briefboard serve`** поднимает доску для текущей папки — помнить про
  ...
  самого проекта, когда этот файл — от briefboard (записан в манифесте или байт в
  байт совпадает с пакетным). `server/server.js`, которого нет в читаемом
  манифесте, она называет, но не запускает, и откатывается на копию из
  установленного пакета; когда читаемого манифеста нет вовсе, копия проекта всё
  равно запускается, а `serve` сообщает, что её происхождение ничем не
  подтверждено. И печатает, что из двух запустила.
...
- **Сначала план.** `briefboard update` без флагов не пишет ничего. По каждому
  ...
  который манифеста не писал, так что сравнивать этот файл не с чем),
  `unknown provenance` (манифест есть, и этого файла в нём нет, — значит,
  briefboard его не устанавливал), `block removed` или `markers malformed`
  (блок briefboard в вашем `CLAUDE.md` / `AGENTS.md` исчез либо его маркеры
  повреждены). `--apply` добавляет каждый файл `new in package` и заменяет
  каждый `outdated` — ради этого обновление и существует. Из пяти категорий, за
  которые briefboard поручиться не может, заменяется только `no manifest`, после
  резервной копии; `MODIFIED LOCALLY`, `unknown provenance` и `block removed`
  сохраняются, пока вы не добавите `--force`, а `markers malformed` не трогается
  и с ним.
- **`doc/` не трогается** ни при `--apply`, ни при `--force`. Бэклог и брифы —
  ваши данные.
- **`CLAUDE.md` и `AGENTS.md` дополняются, а не заменяются.** Если такого файла
  нет, briefboard пишет свой; если он уже есть — дописывает блок между
  `<!-- briefboard:start -->` и `<!-- briefboard:end -->` и не пишет за его
  пределами ни при каком флаге. `update` обновляет только содержимое этого
  блока.
- **Файлы сохраняются по двум разным причинам.** Файл, который briefboard
  установил, а вы потом правили, — это `MODIFIED LOCALLY`. Файл, который был
  вашим с самого начала, — `CLAUDE.md` проекта, у которого он уже был, — это
  `unknown provenance`: в манифесте его нет, и briefboard его туда не клал. Оба
  `--apply` оставляет как есть; `--force` заменит и тот и другой, сделав
  резервную копию.
...
node tools/task.mjs start T-0007 [--json]
                                  # командная форма дропа из Ready в In Progress: переводит
                                  # задачу в in_progress и запускает воркер-сессию в её
                                  # собственном worktree. Это КЛИЕНТ доски, её
                                  # POST /api/task/:id/start — гейт `ready`, гейт зависимостей
                                  # и worktree принадлежат серверу, второй копии этих правил
                                  # здесь нет
                                  # доска должна РАБОТАТЬ (сессия её не переживает), а без
                                  # BRIEFBOARD_WORKER_CMD команда отказывает ДО отправки, и
                                  # задача остаётся ready. Никакого --force: обход гейта
                                  # зависимостей остаётся за `status … --force`
                                  # у каждого рода отказа свой код возврата, и `reason` в
                                  # --json — та же самая таблица, см. руководство
node tools/task.mjs review-start T-0007 [--json]
                                  # командная форма кнопки «Запустить ревью-сессию» на
                                  # карточке: тот же клиент, направленный на
                                  # POST /api/task/:id/review. Задача должна УЖЕ быть в
                                  # `review`, и команда НЕ МЕНЯЕТ СТАТУС и ничего не сливает —
                                  # сессия читает диф, прогоняет проверки и дописывает секцию
                                  # «### Review verdict», чем она всегда и занималась
                                  # без BRIEFBOARD_REVIEW_CMD (её настраивает и прежний
                                  # BRIEFBOARD_ORCHESTRATOR_CMD) отказывает до отправки и
                                  # делит с `start` его таблицу кодов, а не заводит свою
node tools/task.mjs rework T-0007 [--json]
                                  # командная форма кнопки «Вернуть на доработку» на
                                  # карточке: `review` → `in_progress` И воркер-сессия, на той
                                  # ветке, на которой уже лежит предыдущий круг. Переход был
                                  # разрешён всегда и --force ему не требовался; новое здесь —
                                  # сама отправка, которой `status … in_progress` не делает
                                  # отказывает, когда ветки task/T-0007 нет — доработка начала
                                  # бы с HEAD и потеряла бы тот круг, — со своим кодом возврата;
                                  # отсутствующий WORKTREE вместо этого воссоздаётся
                                  # --json несёт `round` — начинаемый круг, выведенный из уже
                                  # написанных вердиктов и нигде не хранимый
node tools/task.mjs resume T-0007 [--json]
                                  # командная форма кнопки «Продолжить работу» на карточке:
                                  # снова воркер-сессия, на той ветке, на которой задача уже
                                  # стоит, — для карточки в `in_progress`, чья сессия пропала.
                                  # Статуса НЕ пишет: карточка и так там, где надо, поэтому
                                  # ничего не двигается и ничего не возвращается назад
                                  # отказывает, пока по задаче ДЕЙСТВИТЕЛЬНО идёт сессия, —
                                  # это читается из реестра доски и никогда не угадывается по
                                  # статусу; и когда ветки task/T-0007 нет, как у `rework`
                                  # делит с `start` его таблицу кодов, а не заводит свою
