...
- All task changes go only through the CLI `node tools/task.mjs` (add / status /
  priority / depends / labels / profile / brief / link / note / show / list /
  archive / board / sessions / validate). Run it with no arguments and it prints
  the list it actually has.
